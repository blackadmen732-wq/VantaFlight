...
from .preprocess import PixelColorSpace
...
def _angle_in_range(angle: float, limits: tuple[float, float]) -> bool:
    """Check a normalized angle, supporting ranges that cross ±180 degrees."""
    lower, upper = limits
    ...
    def detect(
        ...
        *,
        ...
        if image.ndim != 3 or image.shape[2] != 3:
            raise ValueError("detector expects a three-channel image")
        height, width = image.shape[:2]
        raw_x0, raw_y0, rw, rh = roi or (0, 0, width, height)
        raw_x1 = raw_x0 + max(0, rw)
        raw_y1 = raw_y0 + max(0, rh)
        x0, y0 = max(0, raw_x0), max(0, raw_y0)
        x1, y1 = min(width, raw_x1), min(height, raw_y1)
        if x1 <= x0 or y1 <= y0:
            return []
        crop = image[y0:y1, x0:x1]
        if color_space == PixelColorSpace.BGR:
            hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
        elif color_space == PixelColorSpace.HSV:
            hsv = crop
        else:
            raise ValueError(f"unsupported pixel color space: {color_space}")
        ...
        for profile in self.profiles:
            # Only quadrilaterals have a canonical image/object correspondence.
            if not profile.color_ranges or len(profile.object_points) != 4:
                continue
            ...
            for contour in contours:
                ...
                orientation = float(np.degrees(np.arctan2(edge[1], edge[0])))
                if not _angle_in_range(orientation, profile.orientation_range_deg):
                    continue
