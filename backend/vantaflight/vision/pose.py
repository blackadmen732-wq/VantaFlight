    ...
    def estimate(
        ...
        *,
        ...
        if len(image_points) == 4:
            ...
            if abs(image_area) < 1.0 or object_area * image_area <= 0:
        ...
        projected, _ = cv2.projectPoints(
            object_points, rvec, tvec, self.camera.camera_matrix, self.camera.distortion
        )
        ...
        elif tvec[2] < self.config.min_depth_m:
            reason = "target is behind or too close to camera"
        ...
        raw = PoseEstimate(rvec, tvec, rms, inliers, True, not reason, reason)
