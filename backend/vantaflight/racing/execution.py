...
class SimulationOnlyExecutionGuard:
    ...
    _ALLOWED_SCHEMES = frozenset({"udp", "tcp", "sitl", "sim"})
    ...
    def authorize(
        ...
        if parsed.scheme.lower() not in cls._ALLOWED_SCHEMES:
            raise AutonomousExecutionRejected("endpoint scheme is not approved for SITL")
        ...
        return SimulationExecutionPermit(endpoint=endpoint, adapter_type=adapter_type)
